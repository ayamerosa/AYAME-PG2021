...
//#endif
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		glClearColor(0.5f, 0.2f, 0.4f, 1.0f); //cor de fundo
		glClear(GL_COLOR_BUFFER_BIT);

		//aqui recupera o tamanho atual da janela, para correção do aspect ratio mais tarde
		glfwGetFramebufferSize(window, &width, &height);
		glViewport(400, 300, 400, 300); //direita superior
		...
		glUniform1i(rasterCodeLoc, 1); //um é contornado
		glDrawArrays(GL_LINE_LOOP, 0, 6);

		glUniform1i(rasterCodeLoc, 0);
		glViewport(0, 300, 400, 300); //esquerda superior
		glBindVertexArray(VAO);
		glDrawArrays(GL_TRIANGLES, 0, 6);
		...
		glUniform1i(rasterCodeLoc, 0);
		glViewport(400, 0, 400, 300); //direita inferior
		glBindVertexArray(VAO);
		glDrawArrays(GL_TRIANGLES, 0, 6);
		glUniform1i(rasterCodeLoc, 1);
		glDrawArrays(GL_LINE_LOOP, 0, 6);
		...
		glUniform1i(rasterCodeLoc, 0); 
		glViewport(0, 0, 400, 300); //esquerda inferior
		glBindVertexArray(VAO); 
		glDrawArrays(GL_TRIANGLES, 0, 6);
		glUniform1i(rasterCodeLoc, 1);
		glDrawArrays(GL_LINE_LOOP, 0, 6);
