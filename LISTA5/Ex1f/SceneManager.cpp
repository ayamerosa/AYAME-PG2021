...
void SceneManager::setupScene()
{
	...
	obj->setDimension(glm::vec3(538.0f, 805.0f, 1.0f)); //note que depois podemos reescalar conforme tamanho da sprite
	...
	unsigned int texID = loadTexture("../textures/ayame.png");
...
unsigned int SceneManager::loadTexture(string filename)
{
	...
	if (data)
	{
		...
		for (int i = 0; i < size; i += nrChannels)
		{
			...
			{
				data[i] = 255; //Canal R
				data[i + 1] = data[i]; //Canal G
				data[i + 2] = data[i]; //Canal B
				//data[i + 3] canal alpha, caso tenha 
			}
			else
			{
				data[i] = 0;
