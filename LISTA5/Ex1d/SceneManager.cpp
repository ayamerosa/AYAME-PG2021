...
void SceneManager::setupScene()
{
	...
	obj->setDimension(glm::vec3(750.0f, 566.5f, 1.0f)); //note que depois podemos reescalar conforme tamanho da sprite
...
unsigned int SceneManager::loadTexture(string filename)
{
	...
	if (data)
	{
		...
		{
			data[i] = data[i] ^ 255; // Canal R
			data[i + 1] = data[i + 1] ^ 255; //Canal G
			data[i + 2] = data[i + 2] ^ 255; //Canal B
